...
class RoleActionError(exception.MemebotUserError):
    ...
    def __init__(self, action: str, target_name: str, msg: str = "", *args: Any) -> None:
        """
        ...
        """
        super(RoleActionError, self).__init__(f"Failed to {action} role `@{target_name}`! {msg}", *args)
...
class RoleLocationError(exception.MemebotUserError):
    ...
    def __init__(self) -> None:
        super(RoleLocationError, self).__init__("Unable to access roles outside of a server text channel.")
...
async def role_list(ctx: discord.ext.commands.Context, target_role: Optional[discord.Role]) -> None:
    """
